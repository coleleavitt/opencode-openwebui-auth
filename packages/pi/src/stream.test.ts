...
async function collect(lines: string[]) {
    globalThis.fetch = (async () => sseResponse(lines)) as unknown as typeof fetch;
    const events: Array<{ type: string; delta?: string; content?: string }> = [];
    for await (const event of streamOpenWebUI(MODEL, CONTEXT, { apiKey: "test-token" })) {
        events.push(event as { type: string; delta?: string; content?: string });
    }
    return events;
}
...
test("maps reasoning_content deltas onto pi thinking events before the answer", async () => {
    const events = await collect([
        JSON.stringify({ choices: [{ delta: { role: "assistant", reasoning_content: "17*23 " } }] }),
        JSON.stringify({ choices: [{ delta: { reasoning_content: "= 391" } }] }),
...
test("reads the reasoning and reasoning_text spellings too", async () => {
    const reasoning = await collect([
        JSON.stringify({ choices: [{ delta: { reasoning: "via reasoning" } }] }),
        JSON.stringify({ choices: [{ delta: { content: "ok" }, finish_reason: "stop" }] }),
    ]);
    expect(reasoning.find((event) => event.type === "thinking_end")?.content).toBe("via reasoning");

    const reasoningText = await collect([
        JSON.stringify({ choices: [{ delta: { reasoning_text: "via reasoning_text" } }] }),
        JSON.stringify({ choices: [{ delta: { content: "ok" }, finish_reason: "stop" }] }),
    ]);
    expect(reasoningText.find((event) => event.type === "thinking_end")?.content).toBe(
        "via reasoning_text",
    );
});
...
test("emits no thinking block when the deployment returns empty or redacted reasoning", async () => {
    const events = await collect([
        ...
        JSON.stringify({ choices: [{ delta: { content: "391" }, finish_reason: "stop" }] }),
    ]);

    expect(events.map((event) => event.type)).not.toContain("thinking_start");
    expect(events.find((event) => event.type === "text_end")?.content).toBe("391");
});
